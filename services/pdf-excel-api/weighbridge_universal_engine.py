...
def discover_reports(pages):
    """
    Discover complete reports across all PDF pages.

    A report may span multiple pages. Continuation pages are grouped with
    the current report instead of being discarded.
    """
    reports = []
    current = None
    current_pages = []
    ...
    def flush():
        ...
        combined_lines = split_candidate_lines(combined_text)
        rows, row_indexes = discover_rows(combined_text)
        title = discover_title(combined_lines)
        total = discover_total(combined_lines, row_indexes)

        first_pno, first_text = current_pages[0]
        pm = PAGE_RE.search(first_text)
        page_total = int(pm.group(2)) if pm else current.get("page_total")
        ...
        reports.append({
            ...
            "total": total,
            "rows": rows,
            "pages": [p for p, _ in current_pages],
        })
    ...
    for pno, text in pages:
        ...
        low = text.lower()
        marker = "weighbridge" in low or "daily report" in low

        pm = PAGE_RE.search(text)
        page_no = int(pm.group(1)) if pm else None
        page_total = int(pm.group(2)) if pm else None
        date = dates[0].group(1) if dates else None

        page_rows, _ = discover_rows(text)
        has_content = bool(page_rows)
        ...
        if current is None:
            if date and marker:
                ...
                current_pages = [(pno, text)]
            continue

        current_date = current.get("date")
        current_page_no = current.get("page_no")

        # New report: date changes, or page numbering resets to 1.
        new_report = False
        if date and current_date and date != current_date:
            new_report = True
        elif page_no == 1 and current_page_no not in (None, 1):
            new_report = True
        ...
        # Continuation page: keep its rows/header/total text.
        if has_content or date or marker:
            current_pages.append((pno, text))
            if page_total:
                current["page_total"] = page_total
